...
import { CLAIMS2, RUNG_COLORS2, RUNG_NAMES2 } from '../../../content/act2'
...
export default function ConcludeStage2({ state, dispatch }: { state: LoopState; dispatch: Dispatch<Action> }) {
  const ceil = ceiling(state.rungs, state.act)
  const ceilName = RUNG_NAMES2[ceil]
  const ceilColor = `var(${RUNG_COLORS2[ceil]})`

  return (
    <div className="stage-enter" style={{ maxWidth: 920 }}>
      <Kicker>07 · CONCLUDE, the ladder gates your claim</Kicker>
      <PiBrief step={6} act="differentiate" />
      ...
      <p style={{ color: 'var(--muted)', lineHeight: 1.6, marginBottom: 12 }}>
        Your evidence reaches the <b style={{ color: ceilColor }}>{ceilName}</b> rung. The bench is a model, so
        a claim about Mateo's real tissue sits above any rung you can reach here.
      </p>
      <div style={{ border: '1px solid color-mix(in srgb, var(--c-pink) 30%, var(--line))', borderRadius: 12, background: 'color-mix(in srgb, var(--c-pink) 6%, var(--panel))', padding: '11px 14px', marginBottom: 18 }}>
        <p style={{ fontSize: 13, lineHeight: 1.55, color: 'var(--text)' }}>
          Remember: you moved sliders on a <Define t="model">model</Define> and watched YAP and TAZ shift. You
          did not measure the real force in Mateo's tissue, and you could not, the in-the-body force rung is
          locked. So describe the model, not the patient.
        </p>
      </div>
        ...
        {CLAIMS2.map((c) => {
          ...
          if (sel) {
            if (state.claimResult === 'valid') {
              ...
              verdict =
                c.req >= 4
                  ? "✗ Blocked, that is a claim about Mateo's real tissue. The bench is a model and the in-vivo force rung is locked, so you cannot make it."
                  : '✗ Blocked, this needs the rung-' + c.req + ' tool (reproducibility needs the paid reporter validation), which you did not run.'
              vColor = 'var(--c-pink)'
            }
          }
          return (
            <button
              ...
              style={{
                ...
                background: sel ? (ok ? 'color-mix(in srgb, var(--c-green) 10%, var(--panel))' : 'color-mix(in srgb, var(--c-pink) 9%, var(--panel))') : 'var(--panel)',
                cursor: 'pointer',
                display: 'flex',
                gap: 15,
      ...
      {state.claimResult === 'blocked' && (
        <div style={{ marginTop: 16, border: '1px solid color-mix(in srgb, var(--c-pink) 40%, var(--line))', borderRadius: 10, background: 'color-mix(in srgb, var(--c-pink) 8%, var(--panel))', padding: '11px 14px', fontSize: 13.5, lineHeight: 1.5, color: 'var(--text)' }}>
          That claim is above your evidence. Pick the strongest claim <b>at or below the {ceilName} rung</b>, or go back and hire the interpreter for reporter validation.
        </div>
      )}
      {state.claimResult === 'valid' && (
        <div style={{ marginTop: 18, border: '1px solid color-mix(in srgb, var(--c-green) 35%, var(--line))', borderRadius: 12, background: 'color-mix(in srgb, var(--c-green) 6%, var(--panel))', padding: '14px 16px' }}>
          ...
          <p style={{ fontSize: 13.5, lineHeight: 1.6, color: 'var(--text)', marginBottom: 8 }}>
            They would say exactly what the model showed: in this bench, stiffer and more spread settings push
            YAP and TAZ into the nucleus and bias the cell toward bone. Then they would <b>stop there</b>,
            because a model of the relationship is not a measurement of Mateo, and stiffness is one input among
            several, not a lone cause.
          </p>
          <p style={{ fontSize: 12.5, lineHeight: 1.55, color: 'var(--muted)' }}>
            What you got right: you matched your claim to a model you actually ran and refused the leap to the
            patient. That restraint is the whole job.
          </p>
        </div>
      )}
    </div>
