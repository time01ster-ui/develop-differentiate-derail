...
import { HYPOTHESES2 } from '../../../content/act2'
import type { Action, LoopState } from '../../../state/loop'
import Define from '../../Define'
import { PiBrief } from '../../StageChrome'
import { Kicker, StageH1 } from '../ui'

export default function HypothesizeStage2({ state, dispatch }: { state: LoopState; dispatch: Dispatch<Action> }) {
  return (
    <div className="stage-enter" style={{ maxWidth: 880 }}>
      <Kicker>02 · HYPOTHESIZE</Kicker>
      <PiBrief step={1} act="differentiate" />
      <StageH1>Pick the idea you want to test.</StageH1>
      <p style={{ color: 'var(--muted)', lineHeight: 1.6, marginBottom: 10 }}>
        A good <Define t="hypothesis">hypothesis</Define> says HOW you think the switch works and makes a
        prediction the bench could prove wrong. One of these says mechanics drives the choice; one says fate is
        just random; one says only shape matters and stiffness is irrelevant. Pick the one to test.
      </p>
      <div style={{ fontFamily: "'IBM Plex Mono'", fontSize: 12, color: 'var(--muted)', marginBottom: 22 }}>
        New words you'll meet: <Define t="YAP">YAP</Define> · <Define t="RUNX2">RUNX2</Define> ·{' '}
        <Define t="Sox9">Sox9</Define> · <Define t="substrate stiffness">substrate stiffness</Define>
      </div>
